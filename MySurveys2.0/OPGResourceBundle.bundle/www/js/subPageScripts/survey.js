...
var Survey = {
    ...
    uploadOfflineSurveyList : function(){
        cordovaFunction.getData("OfflineSurveyResultsList", function(data) {
                                results = data.value;
                                if(results.length >= 1){
                                   $(".surveyUploadCount").html(results.length);
                                    $.each(results,function(i,value){
                                       $("#surveyHeader li .custom-select ul").append("<li class='normalfont resultList'><label for="+value['id']+" class='normalfont'><input type='checkbox' name="+value['id']+" id="+value['id']+" class='checkbox' />"+value['Name']+"</label></li>");
                                       });
	...
	$('#surveyHeader li').click(function(e) {
		...
		$('.submitButton').on('click touchstart tap',function(e){
			e.stopPropagation();
    ...
    setSurveyList : function(data) {
        ...
        Survey.displayList();
        //alert(localStorage.getItem('popupsurveyid'));
        //alert(app.surveyIdlist);
